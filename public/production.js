...
function windowMenu(title) {
    switch(title) {
        case "Last Results":
            fetch("notlog").then(onResponse).then(lastResults);
            break;
        case "Manage Machines":
            fetch("department").then(onResponse).then(manageMachines);
            break;
        case "Manage Lots":
            fetch("managelots/1").then(onResponse).then(manageLots);
            break;
        case "Week News":
            fetch("notlog").then(onResponse).then(weekNews);
            break;
      }
}
...
function tableLots(event) {
    ...
    fetch("managelots/0"+encodeURIComponent(lot)+"/"+product+"/"+flag).then(onResponse).then(manageLots);
    if (event) {
        event.preventDefault();
    }
}
